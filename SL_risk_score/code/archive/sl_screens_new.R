...
screen_all <- function(Y, X, family, obsWeights, id, nVar=1000,...){
...
screen_glmnet <- function(Y, X, family, obsWeights, id, alpha = 1, minscreen = 2, nfolds = 10, nlambda = 100, nVar = 1000, ...) {
...
## screen based on logistic regression univariate p-value < level
screen_univariate_logistic_pval <- function(Y, X, family, obsWeights, id, minPvalue = 0.1, minscreen = 2, nVar = 1000, ...) {
  ## logistic regression of outcome on each variable
  listp <- apply(X, 2, function(x, Y, family) {
    summ <- coef(summary(glm(Y ~ x, 
                             family = family, weights = obsWeights)))
...
screen_highcor_random <- function(Y, X, family, obsWeights, id, nVar = 1000, ...) {
