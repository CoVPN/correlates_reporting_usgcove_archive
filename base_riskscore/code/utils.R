...
drop_riskVars_with_fewer_0s_or_1s <- function(dat, risk_vars, np) {
  ...
  if(study_name_code == "COVE"){
    ...
    unlink(here("base_riskscore", "output", "drop_riskVars_with_fewer_0s_or_1s.csv"))
        ...
        if ((dat %>% select(matches(risk_vars[i])) %>% unique() %>% dim())[1] == 2) {
          if ((dim(dat %>% filter(get(risk_vars[i]) == 1))[1] < 10) | (dim(dat %>% filter(get(risk_vars[i]) == 0))[1] < 10)){
            ...
            paste0(risk_vars[i], " dropped from risk score analysis as it had fewer than 10 1's or 0's.") %>%
              write.table(file = here("base_riskscore", "output", "drop_riskVars_with_fewer_0s_or_1s.csv"), sep=",", append = TRUE, row.names = F, col.names = F)
          }
        }
      }
  }
  
  if(study_name_code == "COVE" & !file.exists(here("base_riskscore", "output", "drop_riskVars_with_fewer_0s_or_1s.csv"))){
  ...
  if(study_name_code == "ENSEMBLE" & !file.exists(here("base_riskscore", "output", "drop_riskVars_with_fewer_0s_or_1s.csv"))){
...
drop_riskVars_with_high_total_missing_values <- function(X, riskVars) {
  ...
  unlink(here("base_riskscore", "output", "drop_riskVars_with_high_total_missing_values.csv"))
  ...
  for (i in 1:length(riskVars)) {
    ...
    if (percent_NAs > 0.05) {
      ...
      paste0(riskVars[i], " variable has more than 5% values missing and was dropped from risk score analysis.") %>%
        write.table(file = here("base_riskscore", "output", "drop_riskVars_with_high_total_missing_values.csv"), sep=",", append = TRUE, row.names = F, col.names = F)
      covars_highNAvalues <- riskVars[i]
    }
  }
  
  if(!file.exists(here("base_riskscore", "output", "drop_riskVars_with_high_total_missing_values.csv"))){
...
impute_missing_values <- function(X, riskVars) {
  ...
  unlink(here("base_riskscore", "output", "impute_missing_values.csv"))
  covars <- vector()
  ...
  if (length(covars) == 0) {
    ...
    paste("Imputing missing values in following variables: None") %>%
      write.table(file = here("base_riskscore", "output", "impute_missing_values.csv"), sep=",", append = TRUE, row.names = F, col.names = F)
  } else {
