...
getDummy.COVID.E<-function(dat){
  ...
  dat$R0MinorityInd=floor(dat$X/10000)==1
  dat$Region1=floor((dat$X-dat$R0MinorityInd*10000)/1000)==1
  dat$Region2=floor((dat$X-dat$R0MinorityInd*10000-dat$Region1*1000)/100)==1
  dat$HighriskInd=floor((dat$X-dat$R0MinorityInd*10000-dat$Region1*1000-dat$Region2*100)/10)==1
  dat$Senior=dat$X-dat$R0MinorityInd*10000-dat$Region1*1000-dat$Region2*100-dat$HighriskInd*10
...
getDummy.COVID.E.WX<-function(dat){
...
###################### VE(S) curve ##################
   ...
   datnew<-data.frame(S1=Su)
    ...
    for (i in 1:length(WXu)){
      ...
      PY1.SZ1=PY1.SZ1+R.probit.COVID.E.WX(Z=1,S1=Su,WX=WXu[i],varlist=varlist,beta)*fx.s[,i]
      PY1.SZ0=PY1.SZ0+R.probit.COVID.E.WX(Z=0,S1=Su,WX=WXu[i],varlist=varlist,beta)*fx.s[,i]
