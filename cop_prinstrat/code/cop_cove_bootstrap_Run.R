...
B <- 20 # number of bootstrap replicates 1e3
...
ptids.by.stratum=get.ptids.by.stratum.for.bootstrap (dat.wide) 
...
#save(fit2,VE2,file=paste0(save.results.to,"outCOVE_",ps,"_",y.v,".Rdata"))
  ...
  list(low.ci=out$low.ci,high.ci=out$high.ci,Su=out$Su,VE.S=out$VE.S,low.cb=out$low.cb,high.cb=out$high.cb)
}

for (a in assays){
    out=genVE.M.boot(dat.wide,a)
}
