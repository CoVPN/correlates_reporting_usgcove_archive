...
# parm = a named list with indexes
# returns a list containing previously saved output
one_bootstrap <- function(parm){
...
if (!inherits(fit2,'try-error')) {
 ...
 out.VE2<-rbind(out.VE2,as.numeric(VE.S2))
  ...
  out_list <- list(ps = ps, y.v = y.v, 
                   out.seed = index3, Su = Su, 
                   out.beta2 = out.beta2, out.VE2 = out.VE2)
  return(out_list)
}


# generally, I would put this in a params.R script so it can be easily found
# see cor_coxph/code for an example
n_boot <- 1e3

# make a list that contains all parameter combinations
# index1 = day57 vs. day29
# index2 = assays?
# index3 = number of bootstraps
parm <- expand.grid(index1 = 1:2, index2 = 1:6, index3 = 1:n_boot)
parm_list <- split(parm, seq(dim(parm)[1]))

# parallelization with multicore
library(parallel)
n_cores <- parallel::detectCores() # or set fixed number of cores
rslt <- mclapply(parm_list, mc.cores = n_cores, FUN = one_bootstrap)

# parallelization with future
library(future.apply)
plan(multicore) # can configure to work with slurm as well
# if using future then remove set.seed calls in one_bootstrap or you 
# will get a warning
rslt <- future.apply::future_lapply(parm_list, FUN = one_bootstrap, 
                                    future.seed = 1)


# save(out.seed,Su,out.beta2,out.VE2,file=paste0("cop_prinstrat/Result/outCohortboot_",ps,"_",y.v,"_",index,".Rdata"))
