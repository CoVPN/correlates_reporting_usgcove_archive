...
for (a in assays) {        
    ...
    fit.risk=run.svycoxph(f1, design=twophase(id=list(~1,~1), strata=list(NULL,~Wstratum), subset=~TwophasesampInd, data=dat.vac.seroneg))
    
#    f2=update(form.0, as.formula(paste0(marker.name,"~.")))
#    fit.s=nnet::multinom(f2, dat.vac.seroneg, weights=dat.vac.seroneg$wt) 
        
    risks.all.ter[[a]]=if(length(fit.risk)==1) NA else marginalized.risk(fit.risk, marker.name, subset(dat.vac.seroneg,TwophasesampInd==1), categorical.s=T)
}
...
dev.off()    
#
cumsum(summary(survfit(form.s, subset(dat.vac.seroneg, TwophasesampInd==1)), times=x.time)$n.event)
