...
Average follow-up of vaccine recipients (in the Day `r Day` correlates analyses population) starting at 7 days post Day `r Day` visit (not counting the 7 days) is \input{`r pathCoRoutput`/avg_followup_`r study_name`} days.
 

`r if(Day=="57") paste0("Number of breakthrough vaccine cases (in the Day 57 correlates analyses population) with Day 57 ID80 > 660 IU: \\input{", pathCoRoutput, "/num_vacc_cases_highid80_", study_name, "}.")`


\begin{table}[H]
\caption{Deciles of Day `r Day` cID50 titer for the immunogenicity subcohort in the vaccine arm.}
\begin{center}
\setlength{\tabcolsep}{.5ex}
...
\begin{table}[H]
\caption{Summary statistics for the number of days from dose 1 to Day `r Day` visit. (a) The whole immunogenicity subcohort, (b) non-cases in the immunogenicity subcohort, (c) intercurrent cases, (d) primary cases, i.e. cases from the Day 57 correlates analysis population.
}
\begin{center}
\setlength{\tabcolsep}{.5ex}
