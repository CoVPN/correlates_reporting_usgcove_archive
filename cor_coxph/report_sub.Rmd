...
\end{center}
*Baseline covariates adjusted for: baseline risk score, at risk or not, community of color or not. Average follow-up time \input{`r pathCoRoutput`/CoR_mean_followup_time_vacc_`r study_name`} days, maximum follow-up time \input{`r pathCoRoutput`/CoR_max_followup_time_vacc_`r study_name`} days.\\
...
Average follow-up time \input{`r pathCoRoutput`/CoR_mean_followup_time_vacc_`r study_name`} days, maximum follow-up time \input{`r pathCoRoutput`/CoR_max_followup_time_vacc_`r study_name`} days.
Cutpoints:
%Day `r Day` cutpoints:
...
\begin{figure}[H]
    ...
    \caption{Controlled VE as functions of Day `r Day` markers (>=s) among baseline seronegative vaccine recipients with 95\% bootstrap point-wise confidence bands. Histograms of the immunological markers in the vaccine arm are overlaid. lod: lower limit of detection.}
\end{figure}
