...
\begin{itemize}
    ...
    \item Add -Inf and Inf to the beginning and end of the vector of cut points. (This helps avoid an error in the next step) and use Hmisc::cut2 to trichotomize and convert the results into a factor
    \item If the previous fails (this could happen when two cut points are the same, or the smaller one equals the minimum marker value, or the bigger one equals the maximum marker value), use cut() with breaks=3 to trichotomize
