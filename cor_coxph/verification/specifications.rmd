...
There are four types of curves based on marginalized risks. The first three are for continuous markers and the risk here refers to the marginalized cumulative risk by $t_F$, which is defined as the time of the last observed outcome in the vaccine arm.


(a) Marginalized risk function conditional on $S=s$ for a continuous marker:
\begin{enumerate}
  \item Fit a two-phase Cox model to the baseline negative vaccine recipients with baseline covariates and the marker
  \item Compute the survival probability for all phase 2 subjects, but with their marker value set to $s$ instead of their original $S$
  \item Return the weighted mean of the estimated survival probabilities
\end{enumerate}

(b) Marginalized risk function conditional on $S>=s$ for a continuous marker:
\begin{enumerate}
  ...
  \item Compute the survival probability for all phase 2 subjects
  \item Return the weighted mean of the estimated survival probabilities
\end{enumerate}

(c) Controlled VE function conditional on $S=s$ for a continuous marker:
\begin{enumerate}
  \item Fit a two-phase Cox model to the baseline negative vaccine recipients with baseline covariates using the subset of samples satisfying $S>=s$
  \item Compute the survival probability for all phase 2 subjects
  \item Return  1-  (weighted mean of the estimated survival probabilities) / (placebo overall risk)
\end{enumerate}

The last is for discrete markers, and it compute the marginalized cumulative incidence rate as a function of time for each subpopulation defined by the marker

(a) Marginalized cumulative incidence rate function conditional on $S=s$ for a discrete marker:
\begin{enumerate}
  \item Fit a two-phase Cox model to the baseline negative vaccine recipients with baseline covariates and the marker
  \item Compute the survival probability for all phase 2 subjects, but with their marker value set to $s$ instead of their original $S$
  \item Return the weighted mean of the estimated survival probabilities
