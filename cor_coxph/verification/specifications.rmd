...
\begin{itemize}
    \item Phase 1 ptids in Day 57 analyses are defined by !is.na(wt); in Day 29 analyses they are defined by !is.na(wt.2)
    \item Phase 2 ptids in Day 57 analyses are defined by TwophasesampInd; in Day 29 analyses they are defined by TwophasesampInd.2
\end{itemize}
...
\begin{itemize}
    \item Use Hmisc::wtd.quantile to find cut points at 1/3 and 2/3 quantiles using weights
    \item If the two cut points are the same, or the smaller one equals the minimum marker value, or the bigger one equals the maximum marker value, use cut() with breaks=3 to trichotomize
    \item Otherwise, add -Inf and Inf to the beginning and end of the vector of cut points. (This helps avoid an error in the next step) and use Hmisc::cut2 to trichotomize and convert the results into a factor
...
The subgroups are as follows. For some subgroups we cannot adjust for all baseline demographic variables, e.g. in the at risk subgroup we cannot adjust for HighRiskInd.
