...
\begin{itemize}
    \item Phase 1 ptids in Day 57 analyses are defined by !is.na(wt.D57); in Day 29 analyses they are defined by !is.na(wt.D29)
    \item Phase 2 ptids in Day 57 analyses are defined by TwophasesampIndD57; in Day 29 analyses they are defined by TwophasesampIndD29
\end{itemize}
...
\begin{itemize}
    \item If more than the top 1/3 of vaccine recipients have value > ULOQ, use the cut function to cut at the weighted median (Hmisc:wtd.quantile) among those <= ULOQ and ULOQ; else use the cut function to cut at the 1/3 and 2/3 quantiles (wtd.quantile). (add -Inf and Inf to the beginning and end of the vector of cut points). Convert the results into a factor 
    \item If the previous step fails or if it succeeds but it produces only two categories, use cut() with breaks=3 to trichotomize
