...
for (bstatus in 0:1) {
  ...
  if (all(ve_dat$event == 0)) {  ## no events in the strata
...
#    } else {
    ...
    load(paste0("../cor_coxph/output/", attr(config,"config"),"/PrimaryD57/marginalized.risk.no.marker."%.%study_name%.%".Rdata"))
    VE <- overall.ve[1]
    VE_lb <- overall.ve[2]
    VE_ub <- overall.ve[3]        
#    }
    ...
    # use ve computed as part of cor_coxph, which is defined based on marginalized risks
    load(paste0("../cor_coxph/output/", attr(config,"config"),"/PrimaryD29/marginalized.risk.no.marker."%.%study_name%.%".Rdata"))
    VE <- overall.ve[1]
    VE_lb <- overall.ve[2]
    VE_ub <- overall.ve[3]        
