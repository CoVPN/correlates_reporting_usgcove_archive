...
dat.ph1 <- COVIDcorr::dat.mock %>%
  ...
  select(Ptid, Trt, all_of(endpoint), wt) %>%  
  # Pull in risk_score for vaccinees
  left_join(read.csv(paste0(here(), "/SL_risk_score/output/vaccine_ptids_with_riskscores.csv")) %>% select(Ptid, risk_score), by = "Ptid") %>%
...
saveRDS(cvaucs, "results/cvsl_vacc_riskscore_cvaucs.rds")
save(cvfits, file = "results/cvsl_vacc_riskscore_cvfits.rda")
