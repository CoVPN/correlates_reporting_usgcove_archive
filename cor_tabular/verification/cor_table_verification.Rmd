...
dat.mock <- read.csv(here("verification","practice_data.csv"), stringsAsFactors = F)
...
load(here("verification","Tables_verificatiton_cor.Rdata"))
```
...
for (i in c("Negative", "Positive")){
  ...
  if (i=="Negative"){
    ...
    rr_table1 = tab_dm_neg[, c("subgroup","Characteristics")] %>% left_join(rr_table1, by=c("subgroup","Characteristics"))
    testthat::expect_equal(rr_table1, tab_dm_neg)
    
  } else if (i=="Postive"){
    # comparison
    tab_dm_pos[] <- lapply(tab_dm_pos, as.character)
    rr_table1 = tab_dm_pos[, c("subgroup","Characteristics")] %>% left_join(rr_table1, by=c("subgroup","Characteristics"))
    testthat::expect_equal(rr_table1, tab_dm_pos)
  }
...
```{r table3, results = "asis", echo=FALSE, warning=FALSE, message=F}
fig_footer <- c("")
options(survey.lonely.psu="adjust") 
...
for (mk in c("Anti N IgG (IU/ml)","Anti RBD IgG (IU/ml)","Anti Spike IgG (IU/ml)","Pseudovirus-nAb ID50","Pseudovirus-nAb ID80")){
  ...
  wt_var = ifelse(tp==two_timepoints[1], "wt.2", "wt")
...
rr3_table_all <- rr3_rate_all %>%
  ...
  filter(! (Baseline=="Negative" & Trt_lb=="Placebo")) %>%
  select(1:4,5,7,9,6,8,10,11,12)

case_vacc_neg$Baseline="Negative"
case_vacc_neg$Trt_lb="Vaccine"

case_plcb_pos$Baseline="Positive"
case_plcb_pos$Trt_lb="Placebo"

case_vacc_pos$Baseline="Positive"
case_vacc_pos$Trt_lb="Vaccine"

all_cases=rbind(case_vacc_neg, case_plcb_pos, case_vacc_pos) %>%
  select(1,11,12,2,3:10)

dim(rr3_table_all)==dim(all_cases) 

all_cases[] <- lapply(all_cases, as.character)
rr3_table_all[] <- lapply(rr3_table_all, as.character)

rr3_table_all <- all_cases[, c("Visit","Baseline","Trt_lb","Marker")] %>%
  left_join(rr3_table_all, by=c("Visit","Baseline","Trt_lb","Marker"))

testthat::expect_equal(all_cases, rr3_table_all)
```
