...
2. Subset to keep the cohort identified for the correlates of risk at Day 57(`!is.na(wt)`).
...
5. Calculate the weighted (`wt`) ratios of GMTs/GMCs and 95% CI between Cases vs Non-cases participants, for all markers by visit, case/non-case status, arm, baseline COVID status, and marker: run the model `survey::svyglm(magnitude ~ case, svydesign(ids = ~ Ptid, strata = ~ Wstratum, weights = ~ wt, data = data))`, where `magnitude` represents the log10-magnitude endpoints, `case` is the indicator defined in step 1, and `data` contains the participants with `wt` only. Pull the estimate and 95% CI of the coefficient of `case`, then 10 power the results to the linear scale. The 95% CI used `base::confint()`.
