...
2. Subset the data to "phase 1 ptids" for the correlates at Day 57 cohort (`Perprotocol == 1 & EventTimePrimaryD57 >= 7`). Derive the indicator for the correlates of risk at Day 57(`!is.na(wt)`).
...
2. Calculate the weighted (`wt`) proportion and 95% CI of responder, for all markers by visit, case/non-case status, arm, baseline COVID status and marker. The estimation used `prop.est <- survey::svyciprop(~ response, twophase(ids = list(~Ptid, ~ Ptid), strata = list(NULL, ~ Wstratum), weights = list(NULL, ~ wt), subset=~corrset, method="simple", data = data))`, where `data` contains **all** participants, `corrset` is the indicator of the correlates of risk cohort at Day 57 defined in Initial data processing step 2. The 95% CI is extracted from the attributes `attributes(prop.est)$ci`. The responder rates are shown with the counts of responders (n) and the counts of the subgroup (N, each subgroup is a combination of visit, case/non-case status, arm, baseline COVID status, and marker) in the format of "n/N=pct%". Here n & N are weighted: n=sum(response * `wt`); N=sum(`wt`).
3. Calculate the weighted (`wt`) GMTs/GMCs and 95% CI for all markers by visit, case/non-case status, arm, baseline COVID status and marker: calculate the mean and 95% CI of the log10-magnitudes of the markers, then 10 power the results to the linear scale. The estimation of the mean of the log10-magnitudes used `survey::svymean(~ magnitude, svydesign(ids = ~ Ptid, strata = ~ Wstratum, weights = ~ wt, method="simple", data = data))`, `data` contains **all** participants, `corrset` is the indicator of the correlates of risk cohort at Day 57 defined in Initial data processing step 2. The 95% log10-scaled CI used `base::confint()`.
4. Calculate the differences and 95% CI of response rates between Cases vs Non-cases participants, for all markers by visit, case/non-case status, arm, baseline COVID status, and marker: Format the table from step 2 into wide format by case/non-case status, calculate the response rate difference and 95% CI limits using the equation in Table 8 step 1, where $p_{1}$ is the weighted reponse rate of Case participants, $p_{2}$ is the weighted response rate of baseline COVID Non-Cases participants, $L_{1}, U_{1}$ are the 95% CI limits $p_{1}$, and $L_{2}, U_{2}$ are the 95% CI limits of $p_{2}$.
5. Calculate the weighted (`wt`) ratios of GMTs/GMCs and 95% CI between Cases vs Non-cases participants, for all markers by visit, case/non-case status, arm, baseline COVID status, and marker: run the model `survey::svyglm(magnitude ~ case, twophase(ids = list(~Ptid, ~ Ptid), strata = list(NULL, ~ Wstratum), weights = list(NULL, ~ wt), subset=~corrset, method="simple", data = data))`, where `magnitude` represents the log10-magnitude endpoints, `case` is the indicator defined in step 1, and `data` contains **all** participants, `corrset` is the indicator of the correlates of risk cohort at Day 57 defined in Initial data processing step 2. Pull the estimate and 95% CI of the coefficient of `case`, then 10 power the results to the linear scale. The 95% CI used `base::confint()` or `svyby(..., vartype="ci")`.
...
7. Subset the table into Baseline SARS-CoV-2 Negative Vaccine Recipients (Table 3), Baseline SARS-CoV-2 Positive Vaccine Recipients (Table 4), and Baseline SARS-CoV-2 Positive Placebo Recipients (Table 5).

# Table 3-5. Antibody level comparison of Cases vs Non-Cases by baseline COVID status and assigned arms at Day 29
1. Read in the practice data correlates_reporting/data_clean/practice_data.csv.
2. Subset the data to "phase 1 ptids" for the correlates at Day 29 cohort (`(EventTimePrimaryD29 >= 14 & Perprotocol == 1)|(EventTimePrimaryD29 >= 7 & EventTimePrimaryD29 <= 13 & Fullvaccine == 1)`). Derive the indicator for the correlates of risk at Day 29(`!is.na(wt.2)`).
3. Repeat the steps of Table 3-5 for correlates cohort at Day 57 with weights `wt.2` and stratum `Wstratum`. 
