...
for(marker in assays) {
  ...
  write.csv(esttmle, file.path("input", "threshold_searching",
  ...
  kable(esttmle_table[index_to_show,c(1,2,3,4,5)], row.names = F, format="latex", booktabs=TRUE) %>% kable_styling(latex_options = c("scaled_down", "striped"), )%>%
    save_kable(file = file.path("input", "threshold_searching",
                           paste0("tmleThresh_TABLE", marker, "_pointwise.png")))
  kable(esttmle_table[index_to_show,c(1,2,3,6,7)], row.names = F, format="latex", booktabs=TRUE) %>% kable_styling(latex_options = c("scaled_down", "striped"), )%>%
    save_kable(file = file.path("input", "threshold_searching",
...
for(marker in names(output_list)) {
  ...
  plot <- v + scale_x_continuous(n.breaks = 7,
                         labels = trans_format("ident", math_format(10^.x)), limits = c(min(esttmle[,1])-0.1, max(esttmle[,1])+0.1))   + xlab(paste0(marker, " threshold")) +ylab(laby)+ theme(axis.text.x = element_text(angle = 0, hjust = 1)) + xlab(labx) + ggtitle(paste0(main," with point-wise confidence intervals")) +
  ...
  ggsave(filename = file.path("input", "threshold_searching",
  ...
  plot <- v + scale_x_continuous(n.breaks = 7,
                                 labels = trans_format("ident", math_format(10^.x)), limits = c(min(esttmle[,1])-0.1, max(esttmle[,1])+0.1))   + xlab(paste0(marker, " threshold")) +ylab(laby)+ theme(axis.text.x = element_text(angle = 0, hjust = 1)) + xlab(labx) + ggtitle(paste0(main," with simultaneous confidence bands")) +
    stat_function(fun = RCDF, color = col,  geom = "area", fill = col, alpha = 0.2) +
    scale_y_continuous(
      name = laby,
      sec.axis = sec_axis(~./scale_coef, name="Reverse CDF"), n.breaks = 10
  ...
  write.csv(plot$inv_estimates,  file.path("input", "threshold_searching",
  ...
  kable(inv_est ,format="latex", booktabs=TRUE) %>% kable_styling(latex_options = c("scaled_down", "striped"))%>%
    ...
    labels = trans_format("ident", math_format(10^.x))) + scale_x_continuous(n.breaks = 7)  + ylab(paste0(marker, " threshold")) +xlab("Probability of COVID")+ theme(axis.text.x = element_text(angle = 45, hjust = 1)) + ylab(labx) +theme(plot.title = element_text(size=12))
  ggsave(filename = file.path("input", "threshold_searching",
