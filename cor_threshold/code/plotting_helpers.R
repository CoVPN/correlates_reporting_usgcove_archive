...
get_plot <- function(marker,simultaneous_CI = F, monotone = F ) {
  ...
  plot <- v + scale_x_continuous(n.breaks = 7,
                                 labels = trans_format("ident", math_format(10^.x)), limits = c(min(esttmle[,1])-0.1, max(esttmle[,1])+0.1))   + xlab(paste0(marker, " threshold")) +ylab(laby)+ theme(axis.text.x = element_text(angle = 0, hjust = 1)) + xlab(labx) + ggtitle(main) +
  ...
  print(folder)
  ggsave(filename = here::here("figs", folder,
...
generate_tables <- function(marker, num_show = 10) {
  ...
  kable(esttmle_table[index_to_show,c(1,2,3,4,5)], format = "latex", row.names = F, booktabs=TRUE) %>% kable_styling(latex_options = c("scaled_down", "striped"), )%>%
    save_kable(file = here::here("figs", "pointwise_CI",
                                 paste0("TABLE_", marker, "_pointwiseCI.pdf")))
  kable(esttmle_table[index_to_show,c(1,2,3,6,7)],  format = "latex", row.names = F, booktabs=TRUE) %>% kable_styling(latex_options = c("scaled_down", "striped"), )%>%
    save_kable(file = here::here("figs",  "simultaneous_CI",
...
get_inverse_plot <- function(marker, simultaneous_CI = F) {
  ...
  write.csv(plot$inv_estimates,  here::here("output",
  ...
  kable(inv_est , format = "latex", booktabs=TRUE) %>% kable_styling(latex_options = c("scaled_down", "striped"))%>%
    save_kable(file = here::here("figs", folder,
                                 paste0("TABLE_INVERSE_", marker,"_", append,".pdf")))
  day <- ""
  if(marker == "Day57liveneutid80") {
    laby <- paste0(day, "Live nAb ID80")
  }
  
  if(marker == "Day57pseudoneutid80") {
    laby <- paste0(day, "PsV -nAb ID80")
  }
  if(marker == "Day57liveneutid50") {
    laby <- paste0(day, " Live nAb ID50")
  }
  
  if(marker == "Day57pseudoneutid50") {
    laby <- paste0(day, "PsV -nAb ID50")
  }
  
  if(marker == "Day57bindRBD") {
    laby <- paste0(day, "RBD IgG (IU/ml)")
  }
  
  if(marker == "Day57bindSpike") {
    laby <- paste0(day, "Spike IgG (IU/ml)")
  }
  ...
  plot <- plot$plot + scale_y_continuous(
    labels = trans_format("ident", math_format(10^.x))) + scale_x_continuous(n.breaks = 7)  +xlab(main)+ theme(axis.text.x = element_text(angle = 45, hjust = 1)) + ylab(laby) +theme(plot.title = element_text(size=12))
  print(plot)
  ggsave(filename = here::here("figs", folder,
