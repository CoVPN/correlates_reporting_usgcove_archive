...
plot_inverse_threshold_response <- function(output, risks = unique(round(seq(min(output[,2]), max(output[,2]), length.out = 10),4)), monotone_decreasing = T, simultaneous_CI = T){
  ...
  if(!simultaneous_CI){
    risks <- mat
    plot_data <- data.frame(cutoffs = round(mat[,1],5), est = mat[,3],
                            lower = (mat[,2]),
                            upper = (mat[,4]))
    ...
    plot_data <- data.frame(cutoffs = round(ps,5), est = out[,2],
                            lower = (out[,1]),
                            upper = (out[,3]))
    low_CI <- out[,1]
    high_CI <- out[,3]
  ...
  g1 <- ggplot2::ggplot(data =  plot_data, aes(x=cutoffs, y=est))+ scale_x_continuous(n.breaks = 10) + scale_y_continuous(n.breaks = 10) +
  ...
  g1 <- g1  +
    theme(text = element_text(size=20),
          axis.text.x = element_text(angle=45, hjust=1)) +ylab("Threshold") + xlab("Risk")
  if(simultaneous_CI){
...
plot_threshold_response <- function(output, simultaneous_CI = T, monotone = F) {
  ...
  if(!is.null(estimates)) {
    library(ggplot2)
    plot_data <- data.frame(cutoffs = round(estimates[,1],2), est = estimates[,2],
                            lower =lower,
                            upper = upper)
    ...
    g1 <- g1  +
      theme(text = element_text(size=20),
            axis.text.x = element_text(angle=45, hjust=1)) +ylab("EWE[Y|A>=v, W]") + xlab("Threshold")
