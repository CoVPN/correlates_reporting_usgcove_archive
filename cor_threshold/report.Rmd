...
ci <- "pointwise"
monotone <- F
include_eval <- "Day57" %in% times && key %in% markers
```

```{r, include = include_eval, eval =  include_eval, child=here::here('cor_threshold','sub_report.Rmd')}
...
ci <- "pointwise"
monotone <- F
include_eval <- "Day29" %in% times && key %in% markers
```
...
ci <- "simultaneous"
monotone <- T
include_eval <- paste0("Day",day) %in% times && key %in% markers
```
