...
for (bb in 1:2) {
  for (tt in c(1, 2, 4)){
    ...
    for (aa in 1:4) {
      rcdf_list[[aa]] <- ggplot(subset(dat.long.twophase.sample, assay == assays[aa] & Bserostatus == bstatus.labels[bb]), 
...
## than 50 data points
dat.sample3 <-  dat.long.twophase.sample[, c("Trt", "Bserostatus", "B", "Day29", "Day57", "Delta29overB", "Delta57overB", "assay", "EventD57")] %>%
  filter(., .$Trt == "Vaccine") %>% 
...
dat.sample4 <-  dat.long[, c("Trt", "Bserostatus", "B", "Day29", "Day57", "Delta29overB", "Delta57overB", "assay", "EventD57")] %>%
  filter(., .$Trt == "Vaccine") %>% 
  split(., list(.$Bserostatus, .$assay, .$EventD57)) %>%
  lapply(., function(x) {
    if(nrow(x) <= 30) {
...
dat.sample5 <-  dat.long[, c("Trt", "Bserostatus", "B", "Day29", "Day57", "Delta29overB", 
