...
labels.axis[, "pseudoneutid50"] <- "Pseudovirus-nAb ID50"
...
# labels.axis=outer(c("D1 ", "D29 ", "D57 ", "D29 Fold-rise over D1 ", "D57 Fold-rise over D1 "),
#                   c("Anti-Spike IgG (IU/ml)", "Anti-RBD IgG (IU/ml)", "Pseudo nAb ID50", "Pseudo nAb ID80"),
#                   "%.%")

# rownames(labels.axis)=times
# colnames(labels.axis)=assays

# labels.title=outer(c("Baseline ", "Day 29 ", "Day 57 ", "Day 29 Fold-rise over Day 1 ", "Day 57 Fold-rise over Day 1 "),
#                   c("Binding Antibody to Spike", "Binding Antibody to RBD", "Pseudo Neutralization 50% Titer", "Pseudo Neutralization 80% Titer"),
#                   "%.%")
...
## the subgroups below are deprecated
# max.stratum2 = 8
# bstrata.labels = c("Age < 65, Non-Minority, Not High Risk",
#                    "Age >= 65, Non-Minority, Not High Risk",
#                    "Age < 65, Non-Minority, High Risk",
#                    "Age >= 65, Non-Minority, High Risk",
#                    "Age < 65, Minority, Not High Risk",
#                    "Age >= 65, Minority, Not High Risk",
#                    "Age < 65, Minority, High Risk",
#                    "Age>=65, Minority, High Risk")
# bstrata.labels2 = c("age_le_65_non_minority_not_high_risk",
#                     "age_geq_65_non_minority_not_high_risk",
#                     "age_le_65_non_minority_high_risk",
#                     "age_geq_65_non_minority_high_risk",
#                     "age_le_65_minority_not_high_risk",
#                     "age_geq_65_minority_not_high_risk",
#                     "age_le_65_minority_high_risk",
#                     "age_geq_65_minority_high_risk")

## adding race labels
# race.labels <- c("White", "Black", "Asian", "NatAmer", "PacIsl", "Multiracial",
#                  "Notreported", "Other", "Unknown")



## redefine the column bstratum through baseline values

# bstratum_dat <- expand.grid(age.geq.65 = c(0, 1),
#                             HighRiskInd = c(0, 1),
#                             MinorityInd = c(0, 1))
# # bstratum_dat$Bstratum2 <- 1:8  ## baseline demographic stratum, defined by age group, minority status and risk group
# # dat.mock <- left_join(dat.mock, bstratum_dat, by = c("age.geq.65", "HighRiskInd", "MinorityInd"))
# # dat.mock$Bstratum2 <- factor(dat.mock$Bstratum2, levels = 1:8)
#



## Set low values of day 0 and day 57 markers to floor values
## Binding Ab: log10(17)
## Neut ID50: log10(25)
## Neut ID80: log(22)
...
## make a subset of data with 30 sample pooints for the jitter in each subgroup defined by Trt:Bserostatus:Bstratum
# dat.mock.sample.strata <- dat.mock.long[, c("Trt", "Bserostatus", "B", "Day57", "Delta", "assay", "Bstratum", "Bstratum2")] %>%
#   filter(., complete.cases(.)) %>%
#   split(., list(.$Trt, .$Bserostatus, .$Bstratum2)) %>%
#   lapply(., function(x) {
#     if(nrow(x) <= 30) {
#       return(x)
#     } else {
...
# dat.mock.sample.strata.demographics <- dat.mock.long[, c("Trt", "Bserostatus", "B", "Day57", "Delta", "assay", "Bstratum", "age.geq.65",
#                                                          "EventIndPrimary", "HighRiskInd")] %>%
#   filter(., complete.cases(.)) %>%
#   split(., list(.$Trt, .$Bserostatus, .$EventIndPrimary, .$HighRiskInd, .$age.geq.65)) %>%
#   lapply(., function(x) {
#     if(nrow(x) <= 30) {
#       return(x)
#     } else {
