...
boxplot_jitter_points <- dat.mock.long.twophase.sample[, c("Trt", "Bserostatus", "B", "Day29",
...
for (tt in 1:5){
  for (trt in 1:2) {
    for (bstatus in 1:2) {
      ...
      for (aa in 1:4) {
        subdat <- subset(dat.mock.long.twophase.sample, assay == assays[aa] & Bserostatus == bstatus.labels[bstatus] & Trt == trt.labels[trt])
...
## =================================================================================================
## make a subset of data with 30 sample points for the jitter in each subgroup defined by Trt:Bserostatus
boxplot_jitter_points <- dat.mock.long.twophase.sample[, c("Trt", "Bserostatus", "B", "Day29", "Day57", 
...
## =================================================================================================
## make a subset of data with 30 sample points for the jitter in each subgroup defined by Trt:Bserostatus
boxplot_jitter_points <- dat.mock.long.twophase.sample[, c("Trt", "Bserostatus", "B", 
...
## =================================================================================================
## make a subset of data with 30 sample points for the jitter in each subgroup defined by Trt:Bserostatus
dat.mock.long.twophase.sample.race <- dat.mock.long.twophase.sample %>%
  filter(!(race == "White" & WhiteNonHispanic == 0))

boxplot_jitter_points <- dat.mock.long.twophase.sample.race[, c("Trt", "race", "Bserostatus", "B", "Day29", "Day57", 
...
for (tt in 2:5){
  for (trt in 1:2) {
    for (bstatus in 1:2) {
      ...
      for (aa in 1:4) {
        subdat <- subset(dat.mock.long.twophase.sample.race, assay == assays[aa] & Bserostatus == bstatus.labels[bstatus] & Trt == trt.labels[trt])
        subdat_jitter <- subset(boxplot_jitter_points, assay == assays[aa] & Bserostatus == bstatus.labels[bstatus] & Trt == trt.labels[trt])
...
## =================================================================================================
## make a subset of data with 30 sample points for the jitter in each subgroup defined by Trt:Bserostatus
boxplot_jitter_points <- dat.mock.long.twophase.sample.race[, c("Trt", "Bserostatus", "B", "Day29", "Day57", 
...
for (bstatus in 1:2){
  sub.dat.long <- subset(dat.mock.long, Trt == "Vaccine" & Bserostatus == bstatus.labels[bstatus])
...
#============================
dat.mock.long.race <- filter(dat.mock.long, !(race == "White" & WhiteNonHispanic == 0))

for (bstatus in 1:2){
  sub.dat.long <- subset(dat.mock.long.race, Trt == "Vaccine" & Bserostatus == bstatus.labels[bstatus])
  for(tt in 2:5) {
    rcdf_list <- vector("list", 4)
    for (aa in 1:4) {
