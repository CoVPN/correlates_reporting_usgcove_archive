...
data.variables_needed<-dat%>%select(Ptid, Trt, MinorityInd, HighRiskInd, Age, Sex, Bserostatus, Fullvaccine, Perprotocol, EventIndPrimaryD29, 
                                    EventIndPrimaryD57, SubcohortInd, age.geq.65, TwophasesampInd, Bstratum, wt, wt.2, wt.subcohort, race, 
                                    ethnicity, EthnicityHispanic, EthnicityNotreported, EthnicityUnknown,WhiteNonHispanic,
...
data.variables_needed_bindSpike<-dat%>%
  select(Ptid, Trt, MinorityInd, HighRiskInd, Age, Sex, Bserostatus, Fullvaccine, Perprotocol, EventIndPrimaryD29, EventIndPrimaryD57, 
         SubcohortInd, age.geq.65, TwophasesampInd, Bstratum, wt, wt.2, wt.subcohort, race, ethnicity, EthnicityHispanic, EthnicityNotreported, 
...
dat1<-rbind(data.variables_needed_bindSpike,data.variables_needed_bindRBD,data.variables_needed_pseudoneutid50,data.variables_needed_pseudoneutid80)

#keep the cohort identified for immunogenicity
dat.long.twophase.sample<-dat1%>%filter(SubcohortInd==1,TwophasesampInd==1,Perprotocol==1)
...
dat.long.twophase.sample<-dat.long.twophase.sample%>%
  ...
  mutate(minority_label=ifelse(WhiteNonHispanic==0,"Comm. of Color",ifelse(WhiteNonHispanic == 1,"White Non-Hispanic","")))
...
#figure 1.3 Pair plots of D57 fold-rise over D1 Ab markers: baseline negative vaccine arm
bnegative_vaccine_long<-dat.long.twophase.sample%>%filter(Trt==1,Bserostatus==0)%>%select(Ptid,assay,Delta57overB)
...
ggplot(bnegative_vaccine_wide, aes(x=Delta57overB.bindSpike, y=Delta57overB.bindRBD)) +
  geom_point()+scale_x_continuous(breaks=c(2,4,6,8),limits=c(0,10))+scale_y_continuous(breaks=c(2,4,6,8),limits=c(0,10))
...
spearman_corr<-dat.long.twophase.sample%>%filter(Trt==1,Bserostatus==0)%>%select(Ptid,assay,Delta57overB,Bstratum,wt.subcohort)
...
spearman_corr_wide<-spearman_corr_wide%>%drop_na(wt.subcohort)
...
spearman_corr_calculation<-function(var1_var2){
  ...
  c<-0
  #keep<-data.frame()
  #var1_var2="Delta57overB.bindSpike|Delta57overB.bindRBD"
  for (i in 1:500) {
...
set.seed(12345)
spearman_corr_calculation("Delta57overB.bindSpike|Delta57overB.bindRBD")
spearman_corr_calculation("Delta57overB.bindSpike|Delta57overB.pseudoneutid50")
spearman_corr_calculation("Delta57overB.bindSpike|Delta57overB.pseudoneutid80")
spearman_corr_calculation("Delta57overB.bindRBD|Delta57overB.pseudoneutid50")
spearman_corr_calculation("Delta57overB.bindRBD|Delta57overB.pseudoneutid80")
spearman_corr_calculation("Delta57overB.pseudoneutid50|Delta57overB.pseudoneutid80")

#reample_matrix<-as.matrix(keep)
#sum(keep==708)/(nrow(keep)*ncol(keep))
#write.csv(reample_matrix,file="D:/reample_matrix.csv")



########################################################################################################

#Figure 1.25: RCDF plots for D57 Ab markers: baseline negative vaccine arm
RCDF_plots_data_neg_long<-dat.long.twophase.sample%>%filter(Trt==1,Bserostatus==0)%>%select(Ptid,assay,Day57,wt.2)%>%drop_na(wt.2)
RCDF_plots_data_pos_long<-dat.long.twophase.sample%>%filter(Trt==1,Bserostatus==1)%>%select(Ptid,assay,Day57,wt.2)%>%drop_na(wt.2)

RCDF<-function(assay_type){
      df<-RCDF_plots_data%>%filter(assay==assay_type)
      ...
      if (start>0){
      for (i in 1:start){print(1)}}
      for (j in (start+1) : (end-1)){
      print(df[which.min(abs(df$Day57-j)),]$r_cum.pct)}
      for (k in end:10){print(0)}
}
...
#Figure 1.32: Boxplots of D29 Ab markers: baseline negative vaccine + placebo arms

#boxplot
boxplots_data_long<-dat.long.twophase.sample%>%select(Ptid,Trt,Bserostatus,assay,Day57)
#baseline negtive placebo arm
boxplots_data_long%>%filter(Trt==0,Bserostatus==0)%>%group_by(assay)%>% summarize(min = min(Day57),
...
boxplots_data_long%>%filter(Trt==1,Bserostatus==0)%>%group_by(assay)%>% summarize(min = min(Day57),
                                                                                  q1 = quantile(Day57, 0.25),
                                                                                  median = quantile(Day57, 0.50),
                                                                                  mean = mean(Day57),
                                                                                  q3 = quantile(Day57, 0.75),
...
#Figure 1.48: Spaghetti plots of Ab markers over time: baseline negative vaccine + placebo arm

Spaghetti_data_long<-dat.long.twophase.sample%>%select(Ptid,Trt,Bserostatus,assay,B,Day29,Day57)
Spaghetti_data_long%>%filter(Trt==0,Bserostatus==0)%>%group_by(assay)%>% summarize(min = min(B),
                                                                                   q1 = quantile(B, 0.25),
...
Spaghetti_data_long%>%filter(Trt==0,Bserostatus==0)%>%group_by(assay)%>% summarize(min = min(Day29),
                                                                                   q1 = quantile(Day29, 0.25),
                                                                                   median = quantile(Day29, 0.50),
                                                                                   mean = mean(Day29),
                                                                                   q3 = quantile(Day29, 0.75),
                                                                                   max = max(Day29))


Spaghetti_data_long%>%filter(Trt==0,Bserostatus==0)%>%group_by(assay)%>% summarize(min = min(Day57),
                                                                                   q1 = quantile(Day57, 0.25),
                                                                                   median = quantile(Day57, 0.50),
                                                                                   mean = mean(Day57),
                                                                                   q3 = quantile(Day57, 0.75),
...
Spaghetti_data_long<-dat.long.twophase.sample%>%select(Ptid,Trt,Bserostatus,assay,B,Day29,Day57)
Spaghetti_data_long%>%filter(Trt==1,Bserostatus==0)%>%group_by(assay)%>% summarize(min = min(B),
                                                                                   q1 = quantile(B, 0.25),
                                                                                   median = quantile(B, 0.50),
                                                                                   mean = mean(B),
                                                                                   q3 = quantile(B, 0.75),
...
Spaghetti_data_long%>%filter(Trt==1,Bserostatus==0)%>%group_by(assay)%>% summarize(min = min(Day29),
                                                                                   q1 = quantile(Day29, 0.25),
                                                                                   median = quantile(Day29, 0.50),
                                                                                   mean = mean(Day29),
                                                                                   q3 = quantile(Day29, 0.75),
...
Spaghetti_data_long%>%filter(Trt==1,Bserostatus==0)%>%group_by(assay)%>% summarize(min = min(Day57),
                                                                                   q1 = quantile(Day57, 0.25),
                                                                                   median = quantile(Day57, 0.50),
                                                                                   mean = mean(Day57),
                                                                                   q3 = quantile(Day57, 0.75),
