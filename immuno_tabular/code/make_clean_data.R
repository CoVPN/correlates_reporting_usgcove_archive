...
# Generate a long format dataset stacking the subgroups, so that Bserostatus*Trt, Bserostatus*Trt*subgroup could be run together.
# ds_all: subgroup="Total", by Bserostatus*Trt, weight=inverse of sampling prob. of the 8 stratums.
#         subgroup="MinorityInd", "HighRiskInd", "Agecat", "Sex", by Bserostatus*Trt*subgroup(respectively), weight=1

# Email from Peter, 12/31/2020
# On page 36 below Table 6, it is said that "For purposes of analysis White is defined as Race=White and Ethnicity=Not Hispanic or Latino.  All of the other Race subgroups are defined
# solely by the Race variable." Does it mean when plotting by race, I should omit the data of white Hispanic and white Latino?
#   
# I think the answer is yes, White will always mean race==White & ethnicity==Not Hispanic or Latino
# This follows what Moderna did.
...
ds1 <- bind_cols(
  ds_all %>% # Truncate at LLOQ and ULOQ
    ...
    mutate_at(grep("50", c(pnAb_v, lnAb_v), value=T), setLOQ, lloq=nAb50_lloq) %>% 
    mutate_at(grep("80", c(pnAb_v, lnAb_v), value=T), setLOQ, lloq=nAb80_lloq), 
...
ds2<- bind_cols(
  ds1, 
  pmap(list(data=replicate(length(c(bAb, pnAb, lnAb)), ds1, simplify = F),
...
ds <- ds2 %>% 
  mutate(subgroup=factor(subgroup, levels=sub_lev, labels=sub_lb),
         Group=factor(subgroup_cat, levels=grp_lev), 
         Baseline=ifelse(Bserostatus==1, "Positive", "Negative")) 

# Add labels
ep_lev <- c("Anti-Spike IgG", "Anti-RBD IgG", "Pseudo nAb ID50", "Pseudo nAb ID80", "Live Virus nAb ID50", "Live Virus nAb ID80")
resp_lb <- expand.grid(time=visits, 
                       endpoint=c(bAb, pnAb, lnAb), 
                       ind=c("FR2", "FR4", "Resp"), 
                       stringsAsFactors = F) %>% 
  mutate(Visit=case_when(time=="B"~"D1", time=="Day29"~"D29", time=="Day57"~"D57"),
         Endpoint=case_when(endpoint=="bindSpike"~"Anti-Spike IgG", endpoint=="bindRBD"~"Anti-RBD IgG", 
                            endpoint=="pseudoneutid50"~"Pseudo nAb ID50", endpoint=="pseudoneutid80"~"Pseudo nAb ID80",
                            endpoint=="liveneutid50"~"Live Virus nAb ID50",  endpoint=="liveneutid80"~"Live Virus nAb ID80"),
         Endpoint=factor(Endpoint, levels=ep_lev),
         ind.lb=case_when(ind=="FR2"~"2-Fold Rise", ind=="FR4"~"4-Fold Rise", ind=="Resp"~"Responder"))

resp_v <- grep("Resp|FR2|FR4", names(ds), value=T)  

ds_resp_l <- pivot_longer(ds, cols=all_of(resp_v), names_to = "responder_cat", values_to = "response") %>% 
  inner_join(resp_lb %>% unite("responder_cat", c(time, endpoint, ind), sep=""), by="responder_cat")

ds_mag_l <- pivot_longer(ds, cols=all_of(c(bAb_v, pnAb_v, lnAb_v)), names_to = "responder_cat", values_to = "mag") %>% 
  inner_join(resp_lb %>%
