...
ds_s <- ds_o %>%
  ...
  mutate(
    RaceEthC = case_when(
      raceC == "White" & ethnicityC == "Not Hispanic or Latino" ~ "White Non-Hispanic",
...
subgrp <- c(
  "Total" = "All participants", "Age65C" = "Age", "HighRiskC" = "Risk for Severe Covid-19", "AgeRiskC" = "Age, Risk for Severe Covid-19",
  "SexC" = "Sex", "AgeSexC" = "Age, sex", "ethnicityC" = "Hispanic or Latino ethnicity", "RaceEthC" = "Race",
  "MinorityC" = "Race and ethnic group", "AgeMinorC" = "Age, Race and ethnic group"
)

ds_long <- ds_s %>%
  pivot_longer(
    cols = c(Age65C, HighRiskC, AgeRiskC, SexC, AgeSexC, ethnicityC, RaceEthC, MinorityC, AgeMinorC),
...
ds_all <- bind_rows(
  ds_long,
  ds_s %>% mutate(subgroup = factor("All participants", levels = subgrp), subgroup_cat = "All participants")
)
...
ds1 <- bind_cols(
  ds_all %>% # Truncate at LLOQ and ULOQ
    ...
    mutate_at(grep("50", c(pnAb_v, lnAb_v), value = T), setLOQ, lloq = .getLLOQ("50")) %>%
    mutate_at(grep("80", c(pnAb_v, lnAb_v), value = T), setLOQ, lloq = .getLLOQ("80")),
...
ds3 <- bind_cols(
  ds2,
  pmap(list(
    data = replicate(length(c(bAb, pnAb, lnAb)), ds2, simplify = F),
    t1 = "B",
    t2 = "Day57",
...
ds_resp_l <- pivot_longer(ds, cols = all_of(resp_v), names_to = "resp_cat", values_to = "response") %>%
  inner_join(labels_all, by = "resp_cat")

ds_mag_l <- pivot_longer(ds, cols = all_of(c(bAb_v, pnAb_v, lnAb_v)), names_to = "mag_cat", values_to = "mag") %>%
  inner_join(labels_all %>% distinct(mag_cat, time, endpoint, Visit, Endpoint, label.long, label.short), by = "mag_cat")


save(ds, ds_s, ds_long, ds_resp_l, ds_mag_l, labels_all, file = here::here("data_clean", "ds_all.Rdata"))
