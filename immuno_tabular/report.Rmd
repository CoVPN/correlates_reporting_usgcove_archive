...
for (i in 1:length(tlf)){
  ...
  pack_row <- if ("pack_row" %in% tlf_args) tlf_labels[["pack_row"]] else NA 
  ...
  if (!is.na(loop)) {
    ...
    for (j in jloop) {
      ...
      if (!is.na(pack_row)) {
        tabjk <- pack_rows(tabjk, index = table(tab.j[pack_row]))
        }
    print(tabjk)
    cat("\n\n\\pagebreak\n")
    }
      ...
      if (!is.na(group_table_col)){
        tab <- mutate_at(tab, group_table_col, as.character)
        tab <- group_table(tab, group_table_col)
      }
      ...
      tabk <- kable(
        ...
        add_header_above(header_above1) %>%
        add_header_above(header_above2) %>%
        footnote(general_title = "", general = footer_i, threeparttable = TRUE, escape = FALSE)
      
      if (!is.na(pack_row)) {
        tabk <- pack_rows(tabk, index = table(tab[pack_row]))
        }
      print(tabk)
      cat("\n\n\\pagebreak\n")
}
