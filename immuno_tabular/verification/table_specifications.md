...
5. Derive indicators of 2 fold-rise (FR2) and 4 fold-rise (FR4) based on the truncated magnitudes for neutralizing antibody ID50 titers (Pseudovirus-nAb ID50, Live virus-nAb MN50) at each post-enrollment visit. FR2 = 1 if the ratio of post/pre >=2 and FR4 = 1 if the ratio >=4. 

6. Derive indicators of >= 2 x LLOD (2llod) and >= 4 x LLOD (4llod) based on the truncated magnitudes for binding antibody markers (Anti N IgG (IU/ml), Anti Spike IgG (IU/ml), Anti RBD IgG (IU/ml)). 2llod = 1 if the magnitude >= 2 x LLOD and 4llod = 1 if the magnitude >= 4 x LLOD.
...
8. Derive the indicators of positive responders: responders aredefined as participants who had baseline values < LLOD with post-enrollment endpoint values above the LLOD; or participants who had baseline values >= LLOD with a 4-fold increase (FR4 = 1).

9. Derive the indicators of cases and non-cases: Cases are defined as per-protocol participants with the symptomatic infection COVID-19 primary endpoint diagnosed starting 7 days after the Day 57 study visit. (`Perprotocol == 1 & EventIndPrimaryD57 == 1`);
...
10. Define the covariate "Hispanic or Latino Ethnicity" as: Hispanic or Latino(`EthnicityHispanic==1`); Not Hispanic or Latino(`EthnicityHispanic==0 & EthnicityNotreported==0 & EthnicityUnknown==0`); Not reported and unknown (`(EthnicityNotreported==1 | EthnicityUnknown==1)`)

11. Define the covariate "Underrepresented Minority Status in the U.S." as White Non-Hispanic(`WhiteNonHispanic==1`); Communities of Color(`WhiteNonHispanic==0`)
...
Tables 3-5 include post-enrollment visits only as pre-enrollment visits don't have responders by the definition of responders.

1. Table 3a: Calculate the weighted (`wt.subcohort`) proportion and 95% CI of responders, participants with concentrations >= 2 x LLOD or >= 4 x LLOD for binding antibody markers (Anti N IgG (IU/ml), Anti Spike IgG (IU/ml), Anti RBD IgG (IU/ml)) by visit, arm, baseline COVID status and marker. The estimation used `prop.est <- survey::svyciprop(~ response, design = twophase(id = list(~Ptid, ~ Ptid), strata = list(NULL, ~ tps.stratum), method="simple", subset=~subcohort, data = data))`, where `response` represents the binary endpoints (responder, 2llod, 4llod), `data` contains **all and only Phase 1** participants, `subcohort` is the indicator of the immunogenicity cohort defined in Initial data processing step 3. 
...
4. Repeat Step 1 - 3 by assigned arms, baseline COVID status and subgroups for Table 2b-2j with `svyby(..., by = ~ subgroups, vartype="ci")`, where the design is same as Table 3a. (The data used in the survey design must be and can only be the "Phase 1" participants without duplicates. The result is different from using subsetted data of the sub-population, or stacked data of all subgroups below and adding the subgroup variable in `svyby(..., by)`, i.e. a long format data of repeated "Phase 1" participants with Age, Sex, etc. covariates combined into one variable).    
  b. Age (<65, >=65)
...
4. Repeat Step 1 - 3 by subgroups listed in Table 3 Step 3 for Table 4b-4j. 
...
1. Table 5a: Calculate the weighted (`wt.subcohort`) GMTs/GMCs and 95% CI for all markers by visit, arm, baseline COVID status and marker: calculate the mean and 95% CI of the log10-magnitudes of the markers, then 10 power the results to the linear scale. The estimation of the mean of the log10-magnitudes used `survey::svymean(~ magnitude, twophase(id = list(~Ptid, ~ Ptid), strata = list(NULL, ~ tps.stratum), subset=~subcohort, method="simple", data = data))`, where `magnitude` represents the log10-manitude endpoints, `data` contains **all and only Phase 1** participants, `subcohort` is the indicator of the immunogenicity cohort defined in Initial data processing step 3. The 95% log10-CI used `base::confint()` or `svyby(..., vartype="ci")`.
...
1. Table 6a: Calculate the weighted (`wt.subcohort`) geometric mean titer ratios (GMTRs) and geometric mean concentration ratios (GMCRs) between post-baseline and baseline and 95% CI for all markers by arm, baseline COVID status and marker: calculate the mean and 95% CI of the log10-magnitude difference between post-baseline and baseline values (Initial data processing Step 6.), then 10 power the results to the linear scale. The estimation of the mean of the log10-magnitude difference used `survey::svymean(~ magnitude, twophase(id = list(~Ptid, ~ Ptid), strata = list(NULL, ~ tps.stratum), subset=~subcohort, method="simple", data = data))`, where `magnitude` represents the log10-manitude differences, `data` contains **all and only Phase 1** participants, `subcohort` is the indicator of the immunogenicity cohort defined in Initial data processing step 3. The 95% CI calculation used `base::confint()` or `svyby(..., vartype="ci")`.
...
1. Calculate the weighted (`wt.subcohort`) ratios of GMTs/GMCs and 95% CI between categories of the subgroups, for all markers by visit, arm, baseline COVID status and marker: run the model `survey::svyglm(magnitude ~ group, twophase(id = list(~Ptid, ~ Ptid), strata = list(NULL, ~ tps.stratum), method="simple", subset=~subcohort, data = data))`, where `magnitude` represents the log10-magnitude endpoints, `group` is the covariate to be compared between its subgroups, `data` contains **all and only Phase 1** participants, `subcohort` is the indicator of the immunogenicity cohort defined in Initial data processing step 3. Pull the estimate and 95% CI of the coefficient of `group`, then 10 power the results to the linear scale. The 95% CI used `base::confint()` or `svyby(..., vartype="ci")` or `svyby(..., vartype="ci")`.
...
2. Calculate the differences and 95% CI of the weighted (`wt.subcohort`) proportion of responders, 2FR = 1, 4FR = 1  for all markers by vaccine arm, baseline COVID status, visit, and marker, between the vaccine vs placebo arm, baseline COVID negative vs positive, and groups listed in Table 7.  
...
3. Similar to Table 7, calculate the weighted (`wt.subcohort`) ratios of GMTs/GMCs and 95% CI between vaccine vs placebo arm, for all markers by visit, baseline COVID status and marker: run the model `survey::svyglm(magnitude ~ Arm, twophase(id = list(~Ptid, ~ Ptid), strata = list(NULL, ~ tps.stratum), method="simple", subset=~subcohort, data = data))`, where `magnitude` represents the log10-magnitude endpoints, `Arm` is assigned arms, `data` contains **all and only Phase 1** participants, `subcohort` is the indicator of the immunogenicity cohort defined in Initial data processing step 3. Pull the estimate and 95% CI of the coefficient of `Arm`, then 10 power the results to the linear scale. The 95% CI used `base::confint()` or `svyby(..., vartype="ci")`.
...
3. Similar to Table 7, calculate the weighted (`wt.subcohort`) ratios of GMTs/GMCs and 95% CI between baseline COVID positive vs negative, for all markers by visit, arm and marker: run the model `survey::svyglm(magnitude ~ Bserostatus, twophase(id = list(~Ptid, ~ Ptid), strata = list(NULL, ~ tps.stratum), method="simple", subset=~subcohort, data = data))`, where `magnitude` represents the log10-magnitude endpoints, `Bserostatus` is the baseline COVID status,`data` contains **all and only Phase 1** participants, `subcohort` is the indicator of the immunogenicity cohort defined in Initial data processing step 3. Pull the estimate and 95% CI of the coefficient of `Bserostatus`, then 10 power the results to the linear scale. The 95% CI used `base::confint()` or `svyby(..., vartype="ci")`.
