...
screen_all_plus_exposure<- function(Y, X, family, obsWeights, id, nVar=1000,...){
...
screen_glmnet_plus_exposure <- function(Y, X, family, obsWeights, id, alpha = 1, minscreen = 2, nfolds = 10, nlambda = 100, nVar = 1000, ...) {
...
## screen based on logistic regression univariate p-value < level
screen_univariate_logistic_pval_plus_exposure <- function(Y, X, family, obsWeights, id, minPvalue = 0.1, minscreen = 2, nVar = 1000, ...) {
  ## logistic regression of outcome on each variable
  listp <- apply(X, 2, function(x, Y, family) {
    summ <- coef(summary(glm(Y ~ x, 
                             family = family, weights = obsWeights)))
...
screen_highcor_random_plus_exposure <- function(Y, X, family, obsWeights, id, nVar = 1000, ...) {
...
SL.gam.2 <- function(Y, X, newX, family, obsWeights, ...){
  fit <- SL.gam(Y = Y, X = X, newX = newX, family = family, obsWeights = obsWeights,
                deg.gam = 2, cts.num = 8, ...)
  return(fit)
} 
...
methods2 <- c("SL.glm","SL.bayesglm", "SL.glm.interaction","SL.glmnet","SL.gam", #"SL.gam.2", "SL.gam.3", "SL.gam.4", "SL.nnet", "SL.ksvm", "SL.polymars", 
